...
var promQLTemplates = map[string]string{
	...
	//node
	"node_cpu_seconds_total_system":  `avg(rate(node_cpu_seconds_total{mode="system",$1}[10m])) by (instance) *100`,
	"node_cpu_seconds_total_user":    `avg(rate(node_cpu_seconds_total{mode="user",$1}[10m])) by (instance) *100`,
	"node_memory_MemTotal_bytes":     `node_memory_MemTotal_bytes{$1}/1024/1024/1024`,
	"node_memory_MemAvailable_bytes": `node_memory_MemAvailable_bytes{$1}/1024/1024/1024`,
	"node_filesystem_size_bytes":     `sum(max by(device)(sum(node_filesystem_size_bytes{mountpoint!~"/var/lib/kubelet/pods.*|/data/docker.*",fstype=~"xfs|ext.*",$1})))/1024/1024/1024`,
	"node_filesystem_avail_bytes":    `sum(node_filesystem_avail_bytes{mountpoint!~"/var/lib/kubelet/pods.*|/data/docker.*",fstype=~"xfs|ext.*",$1})/1024/1024/1024`,
	"node_netstat_Tcp_CurrEstab":     `node_netstat_Tcp_CurrEstab{$1}`,
	"node_sockstat_TCP_tw":           `node_sockstat_TCP_tw{$1}`,
	"node_sockstat_sockets_used":     `node_sockstat_sockets_used{$1}`,
	"processTop_useCpuRate":          `processTop_useCpuRate{$1}`,
	"processTop_useMemRate":          `processTop_useMemRate{$1}`,
	"processTop_writeDiskBytes":      `processTop_writeDiskBytes{$1}/1024`,
	"processTop_readDiskBytes":       `processTop_readDiskBytes{$1}/1024`,
	"processTop_useMemResident":      `processTop_useMemResident{$1}/1024/1024`,
	"processTop_useMemVirtual":       `processTop_useMemVirtual{$1}/1024/1024`,
...
func makeNodeMetricExpr(tmpl string, o monitoring.QueryOptions) string {
	...
	if o.NodeName != "" {
		nodeSelector = fmt.Sprintf(`instance="%s"`, o.NodeName)
	} else {
		nodeSelector = fmt.Sprintf(`instance=~"%s"`, o.ResourceFilter)
	}
