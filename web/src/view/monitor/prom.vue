<template>
  <div class="cluster">
    <div class="search-bar">
      <div class="title">
        <el-button class="add-cluster-button-import" type="primary" @click="showCreateProm()" size="small">新建</el-button>
        <el-input class="title-input" v-model="queryData.name" placeholder="输入名称" style="width: 200px;"></el-input>
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div>
                <el-button type="primary" @click="showUpdateProm(scope.row)" size="mini">
                  编辑<i class=" el-icon--right"></i>
                </el-button>
                <el-button type="primary" @click="delProm(scope.row)" size="mini">
    ...
    <el-dialog :title="dialogFormTitle" :visible.sync="dialogFormVisible" width="450px" :close="handCreateDialogClose">
      <el-form :model="formFields" ref="formFields" :rules="formRules" label-width="100px" class="demo-form-inline">
        <el-form-item label="名称" prop="name" >
          <el-input v-model.trim="formFields.name" placeholder="输入名称"  />
        </el-form-item>      
        <el-form-item label="地址" prop="url">
          <el-input v-model="formFields.url"  placeholder="输入地址" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false">取 消</el-button>
        <el-button type="primary" @click="createProm('formFields')" :disabled="isSubmiting" :loading="isSubmiting"><span v-show="!isSubmiting">确 定</span><span v-show="isSubmiting">创建中</span></el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog :title="editdialogFormTitle" :visible.sync="editdialogFormVisible" width="450px" :close="handEditDialogClose">
      <el-form :model="formEdit" ref="formEdit" :rules="formRules" label-width="100px" class="demo-form-inline">
        <el-form-item label="名称" prop="name" >
          <el-input v-model.trim="formEdit.name" placeholder="输入名称"  />
        </el-form-item>
        <el-form-item label="地址" prop="url">
          <el-input  v-model="formEdit.url"  placeholder="输入地址" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="editdialogFormVisible = false">取 消</el-button>
        <el-button type="primary" @click="editProm('formEdit')" :disabled="isSubmiting" :loading="isSubmiting"><span v-show="!isSubmiting">更 新</span><span v-show="isSubmiting">创建中</span></el-button>
      </div>
    </el-dialog>    
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 创建规则
      showCreateProm: function() {
        this.dialogFormTitle = "添加数据源"
        this.dialogFormVisible = true
        this.dialogFormUrl = "/monitor/addProm"
      },
      createProm: function(formName){
      ...
      // 更新规则
      showUpdateProm: function(evl) {
        this.editdialogFormTitle = "编辑数据源";
        this.editdialogFormVisible = true;
        this.editdialogFormUrl = "/monitor/putProm";
        this.formEdit = deepCopy(evl);
      },
      editProm: function(formName) {
        this.$refs[formName].validate(async (valid) => {
          if (valid) {
            this.isSubmiting = true;
            try {
      ...
      delProm(evl) {
        this.$confirm('是否要删除数据源:\n'+evl.name+' ? 删除不可找回!!!!', '提示', {
          ...
          await request.post('/monitor/deleteProm', evl);
          this.pageCurrentChange(1);
      ...
      async getPageList() {
        ...
        this.tableData = await request.get('/monitor/getProms', filters);
